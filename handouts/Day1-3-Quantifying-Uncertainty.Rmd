...
Quite obviously, the estimate $\hat{\beta}$ is equal to the actual $\beta$ (if the relationship is indeed linear) plus the additional $(X'X)^{-1}u$ term. That means that in reality, we will even measure the true $\beta$. But this tells us two things:
...
2. Have the term $(X'X)^{-1}u$ as small as possible so that we do stray far away from the true $\beta$
