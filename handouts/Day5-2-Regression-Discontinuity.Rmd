...
```{r}
raw_graders4 <- haven::read_dta("../data/final5.dta")
raw_graders4 <- haven::zap_formats(raw_graders4)
```

Filter the data
```{r}
graders4 <- raw_graders4 %>% 
...
```{r}
f_fit1 <- lm(avgmath ~ tipuach + classize, data=graders4)
f_fit2 <- lm(avgmath ~ tipuach + c_size + classize, data=graders4)
f_fit3 <- lm(avgmath ~ tipuach + c_size + c_size2 + classize, data=graders4)
...
```{r}
frd_fit1 <- felm(avgmath ~ tipuach | 0 | (classize ~ func1) | schlcode, data=graders4)
frd_fit2 <- felm(avgmath ~ tipuach + c_size | 0 | (classize ~ func1) | schlcode, data=graders4)
frd_fit3 <- felm(avgmath ~ tipuach + c_size + c_size2 | 0 | (classize ~ func1) | schlcode, data=graders4)
...
```{r}
sub_graders4 <- filter(graders4, disc == 1)
frd_fit4 <- felm(avgmath ~ tipuach | 0 | (classize ~ func1) | schlcode, data=sub_graders4)
frd_fit5 <- felm(avgmath ~ tipuach + c_size | 0 | (classize ~ func1) | schlcode, data=sub_graders4)
...
Of course, this is a local average treatment affect LATE.
