...
This can probably best be explained by examples. 
...
The regression tries to find an $\hat{a}_0$ and $\hat{a}_1$ that minimizes tries to explain as much as possible of $y$. That is simply another way of saying it minimizes the sum of of squared residuals $\hat{u}_i$. Let's simulate this case to make it more accessible.
...
You can see that the estimate $\hat{a}_1$ is not exactly equal $a_1=2$. This is because of all the unmeasured stuff ($v$ and $z$ in $u$) $v$ and $z$ affect $y$ as well and they will adjust y, so that the relation between $y$ and $x$ alone is not exactly $2$ if we try to explain as much of $y$ as we can with $x$!
