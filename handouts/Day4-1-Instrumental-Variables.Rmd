...
1. Non-zero association between $IV$ and $X$.
...
Let's take an example out of @Gelman.2007, pp. 216 to illustrate. This is a randomized-encouragement design. Suppose, we want to estimate the causal effect of wacthing Sesame Street on letter recognition. Think of a randomized experiment with pre-school children. The treatment $T$ is watching Sesame Street, the control is not watching. The outomce is the score on a test of letter recognition. 

Of course, it is not possible to force children to watch a show and whether the watch or not might be related to a confounding variable that also effects letter recognition (such as curiosity maybe.) But, if watching cannot be randomized, what can we do? We can randomize encouragement to watch. Remember an instrument is a variable thought to "randomly" induce variation in the treatment of interest. Of course, we hope that encouragement actually works (point 1: encouragement is associated with actually watching Sesame Street). But that is something we can reasonably check in the data. 
...
The ignorability and exclusion assumptions can be informally exressed as $corr(IV,\epsilon_i)=0$ and $corr(IV,v_i)=0$. Also $\gamma_1$ must be non-zero. 
...
The problem is the first stage of computing predictors ($T ~ IV$). "A weak first stage relationship results in variation of predicted values well below that of the original regressors, producing less efficient estimates than OLS. Weak instruments also produce biased coefficients with highly nonnormal distributions whose tail variation may be much greater than believed, generating empirical rejection rates under the null much greater than nominal size." [@Young.2017, p. 3]

There are tests for weak instruments in the literature. For instance, a classic one is to compute the F-statistic from the first stage regression and see how "strongly" the null of all coefficients explaining $T$ is rejected. However, while still common practice in many applied areas, it has become criticizied latey [@Young.2017]. The reason is that F-Statistic is not well behaved in finite samples in the presence of weak instruments. 

## Large sample asymptotics and finite sample problems.

The standard errors calculated in most packages employing 2SLS, are based on asymptotic theory. In order to arrive at a formula for those standard errors, we need to asume a certain disttribution of the error term and often, we don't know that. But we can make arguments what the distributino will converge to (mostl to a normal distribution) if the sample size becomes large enough. So we can compute what the standard errors look like "in the asymptotic limit" of inifite sample size. However, we obvioulsy have only finite data and as such, it is always a bit of an article of faith using asymptotic standard errors for small samples. We will talk about this in much more detail in a later section. The problem is that 2SLS seems to be much more susceptible to such finite sample deviations then normal OLS. This is an ongoing field of research though [See @Young.2017], so there is no easy remedy for this, except more data and maybe bootstrapping. 


# Excercises

1. Go to [Joshua Angrist's dataverse site](https://dataverse.harvard.edu/dataset.xhtml?persistentId=hdl:1902.1/11288) and download the data necessary to replicate the main instrumental variable analysis in @Angrist.1998b. Describe what the main hypothesis is, what the accompanying research design core assumptions are and replicate the main regression.  
